...
public class BoardTest {
    ...
    public void testLegal() {
        Board B = new Board(6);
        assertTrue("Red can move 1st turn", B.isLegal(RED));
        assertTrue("Red can move 1st turn", B.isLegal(RED, 0));
        assertFalse("Blue can't move 1st turn", B.isLegal(BLUE));
        assertFalse("Blue can't move 1st turn", B.isLegal(BLUE, 0));
        B.addSpot(RED, 1, 1);
        assertFalse("Red cannot move 2nd turn", B.isLegal(RED));
        assertEquals(BLUE, B.whoseMove());
        assertTrue("Blue can move 2nd turn", B.isLegal(BLUE));
        B.addSpot(BLUE, 2, 1);
        assertFalse("Blue cannot move 3rd turn", B.isLegal(BLUE));
        assertEquals(RED, B.whoseMove());
        assertEquals(RED, B.whoseMove());
        B.addSpot(RED, 1, 1);
        assertFalse("Red cannot move 4th turn", B.isLegal(RED));
        assertEquals(BLUE, B.whoseMove());
        assertFalse("Blue cannot select a Red square", B.isLegal(BLUE, 0));
        assertFalse("Blue cannot select a Red square", B.isLegal(BLUE, 2, 1));
        B.undo();
        assertEquals(RED, B.whoseMove());
        B.undo();
        assertEquals(BLUE, B.whoseMove());
    ...
    public void testView() {
        ...
        ConstantBoard C = new ConstantBoard(B);
        B.addSpot(RED, 1, 1);
        assertEquals(2, C.get(0).getSpots());
    }
